...
public class DrawTextPanel extends JPanel  {
	...
	private ArrayList<DrawTextItem> theString;  // Now, it is changed to an ArrayList<DrawTextItem> !
	...
	private class Canvas extends JPanel {
		...
		protected void paintComponent(Graphics g) {
			...
			if (theString != null) {
				for (DrawTextItem str: theString)
					str.draw(g);
			}
		}
	}
	...
	public void doMousePress( MouseEvent e ) {
		if (e.isMetaDown()) { // Here, we can click right click to remove an item.
			removeItem();
			return;
		}
		...
		s.setFont( new Font( "Serif", Font.ITALIC + Font.BOLD, 15 ));  // Default is null, meaning font of canvas.
		s.setMagnification(3);  // Default is 1, meaning no magnification.
		s.setBorder(true);  // Default is false, meaning don't draw a border.
		//		s.setRotationAngle(17);  // Default is 0, meaning no rotation.
		...
		theString.add(s);  // Setting the string to be drawn on the canvas!
	...
	// This method removes the last item from the canvas one by one.
	private void removeItem() {
		if (theString.size() > 0) {
			theString.remove(theString.size()-1); // remove the last added string
		}
	...
	// This method saves the current canvas into a text file.
	private void saveIm() {
		File save = fileChooser.getOutputFile(this, "Save As", "After work.txt");

		try {
			PrintWriter outPrint = new PrintWriter(save);
			outPrint.println("New text file");
			outPrint.println(canvas.getBackground().getRed());
			outPrint.println(canvas.getBackground().getGreen());
			outPrint.println(canvas.getBackground().getBlue());

			if (theString != null) {
				for (DrawTextItem s: theString) {
					outPrint.println("theString: ");
					outPrint.println(s.getString());
					outPrint.println(s.getX());
					outPrint.println(s.getY());
					outPrint.println(s.getFont().getName());
					outPrint.println(s.getFont().getStyle());
					outPrint.println(s.getFont().getSize());
					outPrint.println(s.getTextColor().getRed());
					outPrint.println(s.getTextColor().getGreen());
					outPrint.println(s.getTextColor().getBlue());
					outPrint.println(s.getTextTransparency());
					...
					if (s.getBackground() == null) {
						...
						outPrint.println(s.getBackground().getRed());
						outPrint.println(s.getBackground().getGreen());
						outPrint.println(s.getBackground().getBlue());
					}

					outPrint.println(s.getBackgroundTransparency());
					outPrint.println(s.getBorder());
					outPrint.println(s.getMagnification());
					outPrint.println(s.getRotationAngle());
				}
			...
			JOptionPane.showMessageDialog(this, "An error occered:" + save);
			System.out.println("Error message: " + e);
		}
	}

	private void openIm() {
		File open = fileChooser.getInputFile(this, "Open File");
		try {
			Scanner in = new Scanner(open);
			if (!in.nextLine().equals("New text file")) {
				JOptionPane.showMessageDialog(this, "File not found" + open);
				return;
			}
			Color savedBackGround = new Color(Integer.parseInt(in.nextLine()), Integer.parseInt(in.nextLine()), Integer.parseInt(in.nextLine()));
			ArrayList<DrawTextItem> newStr = new ArrayList<DrawTextItem>();
			DrawTextItem newIm;
			in.nextLine();  // skipping to the next line.
			
			while (in.hasNext() && in.nextLine().equals("theString: ")) {
				newIm = new DrawTextItem(in.nextLine(), Integer.parseInt(in.nextLine()), Integer.parseInt(in.nextLine()));
				in.nextLine();
				newIm.setFont(new Font(in.nextLine(), Integer.parseInt(in.nextLine()), Integer.parseInt(in.nextLine())));
				newIm.setTextColor(new Color(Integer.parseInt(in.nextLine()), Integer.parseInt(in.nextLine()), Integer.parseInt(in.nextLine())));
				newIm.setTextTransparency(Double.parseDouble(in.nextLine()));
				int red = Integer.parseInt(in.nextLine());
				int green = Integer.parseInt(in.nextLine());
				int blue = Integer.parseInt(in.nextLine());
				
				if (red == -1) {
					newIm.setBackground(null);
				}
				else
					newIm.setBackground(new Color(red, green, blue));
				
				newIm.setBackgroundTransparency(Double.parseDouble(in.nextLine()));
				newIm.setBorder(in.nextBoolean());
				newIm.setMagnification(Double.parseDouble(in.nextLine()));
				newIm.setRotationAngle(Double.parseDouble(in.nextLine()));
				in.nextLine();
				newStr.add(newIm);
			}
			// replace the current background and strings.
			canvas.setBackground(savedBackGround);
			theString = newStr;
		}
		catch (FileNotFoundException e) {
			JOptionPane.showMessageDialog(this, "An error occured" + open);
